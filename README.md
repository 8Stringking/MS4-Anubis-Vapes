...
The target audience is people that already vape, or people that want to stop smoking themselves, but also people who suffer medical conditions who may need to stop smoking. This is also the value to the users, as the target audience can buy these items from Anubis Vapes for those very reasons which are to stop smoking or for medical reasons. The target audience can be people from all ages.
...
When validating the Javascript, it will not pass through jslint without errors. In my efforts to fix these by declaring variables, by extensive research and many attempts, this caused the javascript to cease working properly. In order to leave the site functioning as intended i have had to leave these errors in place, however the javascript where used is in compliance to the bootstrap/django documentation and the same in the boutique ado project.
